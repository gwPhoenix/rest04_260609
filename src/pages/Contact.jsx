...
function ContactInfo({ icon, label, value, href }) {
  return (
    <div className="flex items-start gap-4">
      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-brand-800/10 text-xl dark:bg-brand-800/30">
        {icon}
      </div>
      <div>
        <p className="text-xs font-bold uppercase tracking-widest text-slate-400">{label}</p>
        {href ? (
          <a href={href} className="font-semibold text-slate-800 transition hover:text-brand-800 dark:text-slate-100 dark:hover:text-brand-300">
            {value}
          </a>
        ) : (
          ...
          <div className="grid grid-cols-1 gap-16 lg:grid-cols-5">
            ...
            <div className="lg:col-span-2">
              ...
              <div className="flex flex-col gap-6">
                <ContactInfo icon="✉️" label="이메일" value={company.email} href={`mailto:${company.email}`} />
                <ContactInfo icon="📞" label="전화" value={company.phone} href={`tel:${company.phone}`} />
                <ContactInfo icon="📍" label="주소" value={company.address} />
              </div>
                ...
                <div className="flex gap-3">
                  {company.socialLinks.map(s => (
                    <a
                      ...
                      className="flex h-10 w-10 items-center justify-center rounded-lg bg-brand-50 text-sm font-bold text-brand-800 transition hover:bg-brand-800 hover:text-white dark:bg-brand-950 dark:text-brand-300"
                    >
                      {s.name[0]}
                    </a>
                  ))}
              ...
              {submitted ? (
                <div className="card rounded-2xl p-10 text-center">
                  ...
                  <p className="text-slate-500 dark:text-slate-400">
                    빠른 시일 내에 이메일({form.email})로 답변을 드리겠습니다.
                  </p>
                  <button
                    type="button"
